...
public static class InternalDebug
{
    ...
    [Conditional("DEVELOPMENT_BUILD"), Conditional("UNITY_EDITOR")]
    public static void Log(object message, Color color, bool condition = true)
    {
        if (condition)
        {
            UnityEngine.Debug.Log(string.Format("<color=#" + ColorUtility.ToHtmlStringRGB(color) + ">{0}</color>", message));
        }
    }
